'''
Modified GPU version of FAISS IPS.

Reference:
* https://github.com/texttron/tevatron/blob/main/src/tevatron/faiss_retriever/retriever.py
* https://github.com/texttron/tevatron/blob/main/src/tevatron/faiss_retriever/__main__.py
'''
import pickle
import numpy as np
import glob
...
class FaissRetriever(BaseFaissIPRetriever):
    def to_gpu(self, verbose=False):
        ngpu = faiss.get_num_gpus()
        if verbose:
            logger.info(f'Number of GPUs: {ngpu}.')
        if ngpu > 0:
...
def to_marco(rank_fpath):
    with open(rank_fpath) as f:
        lines = f.readlines()
    ...
    with open(rank_fpath + '.marco', 'w') as f:
        for qid in qq:
...
def main():
    ...
    retriever = FaissRetriever(p_reps_0)
    ...
    for p_reps, p_lookup in shards:
        retriever.add(p_reps.astype(np.float32))
        look_up += p_lookup
